...
def placeholder_inputs(batch_size, num_points, num_steps, state_size):
    ...
    shapefeat_pl = tf.placeholder(tf.float32, shape=(batch_size, 9))
    cur_state_pl = tf.placeholder(tf.float32, shape=(batch_size, state_size))
    return pointcloud_pl, vel_pl, angvel_pl, pos_pl, rot_pl, shapefeat_pl, cur_state_pl
...
def get_model(cur_state, shape_feat, vel, angvel, cell_type, num_cells, hidden_size, dropout_keep_prob, is_training, bn_decay=None, return_init=False):
    ...
    zero_state = rnn_cell.zero_state(batch_size, dtype=tf.float32)

    # feed through RNN
    # outputs are [batch, time_steps, hidden_size]
    outputs, state = tf.nn.dynamic_rnn(rnn_cell, inputs, initial_state=cur_state, dtype=tf.float32)
    
    y = tf.matmul(tf.reshape(outputs, [batch_size*time_steps, hidden_size]), W_hy) + b_hy
    y = tf.reshape(y, [batch_size, time_steps, num_params])

    if return_init:
        return y, state, zero_state
    else:
        return y, state
