...
parser.add_argument('--model', default='step_rnn_single', help='Model name [default: step_rnn_single]')
...
def evaluate():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):            
            pointcloud_pl, vel_pl, ang_pl, pos_pl, roty_pl, shapefeat_pl, cur_state_pl = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT, 1, NUM_UNITS)
            ...
            # Shape processing model ( will only call this once )
            shape_feat = MODEL.get_shape_model(pointcloud_pl, is_training_pl, bn_decay=None)

            # state prediction ( will call this many times )
            if CELL_TYPE=='fc':
                gaussian_params, cur_state = MODEL.get_model(cur_state_pl, shapefeat_pl, vel_pl, ang_pl, CELL_TYPE, NUM_CELLS, NUM_UNITS, 1.0, is_training_pl, bn_decay=None, return_init=True)
                zero_state = tf.zeros_like(cur_state_pl) # has no memory
            else: 
                gaussian_params, cur_state, zero_state = MODEL.get_model(cur_state_pl, shapefeat_pl, vel_pl, ang_pl, CELL_TYPE, NUM_CELLS, NUM_UNITS, 1.0, is_training_pl, bn_decay=None, return_init=True)
            # build gaussians
            num_distr = gaussian_params.get_shape()[2].value / 2
            loc = gaussian_params[:,:,0:num_distr]
            scale = tf.exp(gaussian_params[:,:,num_distr:])
        ...
        ops = {'cur_state_pl' : cur_state_pl,
               'pointcloud_pl': pointcloud_pl,
               'shapefeat_pl' : shapefeat_pl,
               'vel_pl' : vel_pl,
               'ang_pl' : ang_pl,
               'pos_pl' : pos_pl,
               'roty_pl' : roty_pl,
...
def eval_one_epoch(sess, ops, split='test'):
    ...
    while DATASET.has_next_batch(split=split):
        ...
        # get shape feature (same for all steps)
        feed_dict = {ops['pointcloud_pl']: cur_pointcloud_batch, ops['is_training_pl']: is_training}
        shape_feat, np_state = sess.run([ops['shape_feat'], ops['init_state']], feed_dict=feed_dict)

        num_steps = batch_com.shape[1]
        # pass in inital state to start
        cur_com_state = np.copy(cur_com_batch[:,0,:]).reshape((bsize,1,-1))
        cur_rot_state = np.copy(cur_roty_batch[:,0,:]).reshape((bsize,1,-1))
        cur_vel_state = np.copy(cur_vel_batch[:,0,:]).reshape((bsize,1,-1))
        cur_ang_state = np.copy(cur_ang_batch[:,0,:]).reshape((bsize,1,-1))

        sampled_states = []
        sampled_states.append(np.concatenate((cur_vel_batch[:,0,:], cur_ang_batch[:,0,:], cur_com_batch[:,0,:], cur_roty_batch[:,0,:]), axis=1))
        
        # print(sampled_states[0])
        # print(cur_com_batch[:,0,:])
        
        for i in range(num_steps-1):
            feed_dict = {ops['cur_state_pl'] : np_state,
                        ops['shapefeat_pl'] : shape_feat,
                        ops['pos_pl'] : cur_com_state,
                        ops['roty_pl'] : cur_rot_state,
                        ops['vel_pl'] : cur_vel_state,
                        ops['ang_pl'] : cur_ang_state,
                        ops['is_training_pl']: is_training}
            
            np_state, means, devs = sess.run([ops['cur_state'], ops['means'], ops['stds']], feed_dict=feed_dict)

            # build distributions and sample from them to get next state
            if SAMPLE_MEAN:
                sample = means
            else:
                sample = np.random.normal(loc=means, scale=devs)
            # calculate new state
            cur_vel_state += sample[:,:,:2]
            cur_ang_state += sample[:,:,2]
            cur_com_state += sample[:,:,3:5]
            cur_rot_state += sample[:,:,5]
            # save
            sampled_states.append(sampled_states[-1] + sample[:,0,:])
        ...
        for i, state in enumerate(sampled_states):
            vel_err += np.mean(np.linalg.norm(state[:,0:2] - cur_vel_batch[:,i,:], axis=1)) * DATASET.get_init_vel_normalization()
            angvel_err += np.mean(np.abs(state[:,2] - cur_ang_batch[:,i,:])) * DATASET.get_init_angvel_normalization()
            cur_pos_err = np.mean(np.linalg.norm(state[:,3:5] - cur_com_batch[:,i,:], axis=1)) * DATASET.get_com_normalization()
            pos_err += cur_pos_err
            pos_step_err.append(cur_pos_err)
            cur_rot_err = np.mean(np.abs(state[:,5] - cur_roty_batch[:,i,:])) * DATASET.get_rot_normalization()
            rot_err += cur_rot_err
...
#         test_writer.add_summary(summary, step)
        com_err_sum += pos_err
        roty_err_sum += rot_err
        vel_err_sum += vel_err
        ang_err_sum += angvel_err
        ...
        # print(pos_step_err)
        pos_step_err_sum += np.array(pos_step_err)
        rot_step_err_sum += np.array(rot_step_err)
        ...
        if OUTPUT_PRED:
            ...
            with open(file_out, 'w') as f:
                ...
                cur_com = np.array([[x[0], cur_com_y, x[1]] for x in cur_com])
                # print(cur_com)
                gt_pos_out = create_json_vec_list(cur_com)
                # sampled pos
                sampled_pos = np.array([[x[0, 3]*DATASET.get_com_normalization(), cur_com[0][1], x[0, 4]*DATASET.get_com_normalization()] for x in sampled_states])
                # print(sampled_pos)
                ...
                # sampled rot
                sampled_rot = np.array([[0.0, x[0, 5]*DATASET.get_rot_normalization(), 0.0] for x in sampled_states])
                samp_rot_out = create_json_vec_list(sampled_rot)
                # error
                pos_err_out = float(pos_err)
                rot_err_out = float(rot_err)
